	...
	system::{
		dnd5e::{
			...
			data::{
				...
				AreaOfEffect, Condition, DamageRoll, DamageType, Feature, IndirectCondition,
			},
		},
		SourceId,
	},
	...
	if selected_tags.contains(ActionTag::Attack) {
		...
		panes.push(html! {
			<table class="table table-compact m-0 mb-3">
				...
				<tbody>
					{features.clone().into_iter().filter_map(|(feature_path, feature)| {
		...
		panes.push(html!(<div class="mb-2 border-bottom-theme-muted">
			...
			{features.into_iter().filter_map(|(feature_path, feature)| {
				...
				let check_ability;
				let mut check_bonuses = Vec::<(i32, PathBuf)>::new();
				let mut damage_by_type = MultiMap::<Option<DamageType>, (Roll, Option<PathBuf>)>::new();
				let mut modifiers = MultiMap::<Modifier, PathBuf>::new();
				match &attack.check {
					AttackCheckKind::AttackRoll { ability, proficient } => {
						...
						let ability_modifier_path = PathBuf::from(format!("{} modifier", ability.long_name()));
						check_bonuses.push((modifier, ability_modifier_path.clone()));
						if proficient.evaluate(&state) {
							check_bonuses.push((state.proficiency_bonus(), Path::new("proficiency").to_owned()));
						}
						damage_by_type.insert(None, (Roll::from(modifier), Some(ability_modifier_path)));
					}
					AttackCheckKind::SavingThrow { base, dc_ability, proficient, save_ability } => {
						check_ability = *save_ability;
						check_bonuses.push((*base, Path::new("inheirent").to_owned()));
						if let Some(ability) = dc_ability {
							let modifier = state.ability_scores().get(*ability).score().modifier();
							let modifier_path = PathBuf::from(format!("{} modifier", ability.long_name()));
				...
				for (bonus, modifier, path) in state.attack_bonuses().get_weapon_attack(action) {
					check_bonuses.push((bonus, path.to_owned()));
					if let Some(modifier) = modifier {
						modifiers.insert(modifier, path.to_owned());
					}
				}
				...
				for (roll, damage_type, path) in state.attack_bonuses().get_weapon_damage(action) {
					damage_by_type.insert(*damage_type, (*roll, Some(path.to_owned())));
				}

				let check_bonus = check_bonuses.iter().map(|(value, _)| *value).sum::<i32>();

				let check_modifiers = modifiers.into_iter().map(|(modifier, sources)| {
				...
				Some(html!(<div class="border-bottom d-flex flex-row flex-wrap" {onclick}>
						<div class="d-flex flex-column flex-grow-1">
							{feature.name.clone()}
							<div class="label">
								{crate::data::as_feature_path_text(&feature_path)}
							</div>
						</div>
						<div class="d-flex flex-column flex-grow-1">
							<div class="d-flex flex-row">
								{attack.kind.as_ref().map(|atk_kind| html! {
									<div class="d-flex flex-row flex-grow-1 align-items-baseline" style="font-size: 12px;">
								...
								<div class="d-flex flex-row flex-grow-1 align-items-baseline" style="font-size: 12px;">
									{match attack.check {
										...
										AttackCheckKind::SavingThrow { ..} => html!(<>
											...
											{check_ability.abbreviated_name()}{" "}
											{if check_bonus >= 0 { "+" } else { "-" }}
											{check_bonus.abs()}
										</>),
									}}
								...
								{match &attack.damage {
									...
									Some(DamageRoll { roll, base_bonus, damage_type }) => {
										if *base_bonus != 0 {
											damage_by_type.insert(None, (Roll::from(*base_bonus), None));
										}
										if let Some(roll) = roll {
											damage_by_type.insert(None, (roll.evaluate(&state), None));
										}
										
										let mut damage_rolls = Vec::with_capacity(damage_by_type.len());
										for (dmg_type_override, rolls_by_source) in damage_by_type.into_iter() {
											...
											let tooltip = crate::data::as_feature_paths_html_custom(
												...
												|(roll, source)| {
													let source_or_inheirent = source.as_ref().map(|path| path.as_path()).unwrap_or(Path::new("inheirent"));
													(*roll, source_or_inheirent)
												},
										...
										Some(html!(<div class="d-flex flex-row flex-grow-1 align-items-baseline" style="font-size: 12px;">
