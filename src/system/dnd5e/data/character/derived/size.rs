...
impl SizeFormula {
	pub fn get_random(&self, rand: &mut impl rand::Rng) -> (u32, u32) {
		...
		let height = self.height.base + height_mod;
		let weight_mult = self.weight.multiplier.roll(rand) * height_mod;
		let weight = self.weight.base + weight_mult + self.weight.bonus.roll(rand);
		(height, weight)
	}

	pub fn min_height(&self) -> u32 {
		self.height.base + self.height.bonus.min()
	}

	pub fn max_height(&self) -> u32 {
		self.height.base + self.height.bonus.max()
	}
