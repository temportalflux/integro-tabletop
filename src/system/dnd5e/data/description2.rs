...
mod test {
	...
	mod evaluate {
		use super::*;
		use crate::system::dnd5e::{data::{Ability, character::Persistent}, evaluator::GetAbilityModifier};
		...
		fn simple() {
			...
			let info = Info {
				...
				sections: vec![
					Section {
						title: None,
						content: SectionContent::Body("In addition to {num} dmg on failed {DC} save, take {fire} fire damage.".into()),
						children: vec![],
						format_args: FormatArgs::from(vec![
							("fire", GetAbilityModifier(Ability::Strength), true),
						]),
					}
				],
				format_args: FormatArgs::from(vec![
			...
			let expected = Info {
				...
				sections: vec![
					Section {
						title: None,
						content: SectionContent::Body("In addition to +2 dmg on failed 5 save, take +1 fire damage.".into()),
						children: vec![],
						format_args: FormatArgs::from(vec![
							("fire", GetAbilityModifier(Ability::Strength), true),
						]),
